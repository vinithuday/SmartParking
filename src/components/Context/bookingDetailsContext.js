...
export const BookingDetailsProvider = ({ children }) => {
  ...
  const arrivalDateTimeSetter = (arrivalTime) => setArrivalDateTime(arrivalTime);
  const departureTimeSetter = (departureTime) => setDepartureTime(departureTime);
  const totalPriceSetter = (totalPrice) => setTotalPrice(totalPrice);


  return (
    <BookingDetailsContext.Provider value={{ email,location,selectedSlot,arrivalDateTime,departureTime,totalPrice,emailSetter,locationSetter,selectedSlotSetter,arrivalDateTimeSetter,departureTimeSetter,totalPriceSetter}}>
      {children}
